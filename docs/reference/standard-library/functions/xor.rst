...
Returns the bitwise exclusive OR of two inputs.
...
.. list-table::
   ...
   * - Return value
     - ``ANY_BIT``
     - Each bit set in exactly one of IN1 and IN2. Same type as the inputs.

Description
-----------

On ``BOOL`` inputs, returns ``TRUE`` when exactly one input is ``TRUE``.
On ``BYTE``, ``WORD``, ``DWORD`` and ``LWORD`` inputs, each bit of the
result is set when the same bit is set in exactly one input. ``XOR(a, b)``
is the functional form of the ``XOR`` operator: ``a XOR b``. Both forms are
equivalent.

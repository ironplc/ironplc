...
The limit is a property of the data region rather than of structures, so an
array of a primitive type reaches it the same way. This declaration produces
the same error, because 40000 ``DINT`` elements take one slot each:

...
A string literal is an exception: it has no encoding of its own and takes the
encoding of whatever it is used with, so ``wide := 'abc'`` and
``wide = "abc"`` are both accepted and both store or compare UTF-16LE.

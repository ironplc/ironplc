...
fn compile_user_function(
    ...
    let return_struct_info = ctx
        .struct_vars
        .get(&return_id)
        .map(|info| StructReturnInfo {
            data_offset: info.data_offset,
            total_slots: info.total_slots.raw(),
...
/// Metadata for a structure return value in a user-defined function.
///
/// When a function returns a struct type, the return value occupies multiple
/// slots in the data region. The caller must copy the data from the function's
/// return location to the destination variable after CALL.
#[derive(Clone)]
#[allow(dead_code)]
struct StructReturnInfo {
    /// Byte offset in the data region where the return struct's fields start.
    data_offset: u32,
    /// Total number of 8-byte slots the struct occupies.
    total_slots: u32,
    /// Array descriptor index (treats struct as flat slot array).
    desc_index: u16,
}
...
struct UserFunctionInfo {
    ...
    /// If the function returns a struct type, info about the return struct
    /// in the data region. Used at call sites to copy struct data after CALL.
    #[allow(dead_code)]
    return_struct_info: Option<StructReturnInfo>,

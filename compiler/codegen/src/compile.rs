...
fn compile_program_with_functions(
    ...
    // Compile user-defined function block bodies before assigning
    // program-local variables. This populates ctx.user_fb_types so that
    // assign_variables can resolve user-defined FB instance variables.
    // FB body compilation saves/restores all ctx state.
    //
    // Function IDs start at 2 (0=init, 1=scan). Variable slots for FB bodies
    // are allocated after program-local vars, but we don't know program_var_count
    // yet — we'll fix up var_offset after assigning program locals.
    let mut compiled_fb_bodies: Vec<CompiledFunction> = Vec::new();
    let mut next_function_id: u16 = 2;

    // First pass: compile FB bodies to populate user_fb_types metadata.
    // Use a temporary var_offset starting at a high value; we'll recompile
    // with correct offsets after knowing program_var_count.
    // Actually, a simpler approach: just register the type metadata now
    // with a placeholder var_offset, then fix it up after.
    //
    // Even simpler: compile FB bodies with correct var_offset by deferring
    // to after we know program_var_count, but register the type metadata
    // (field_indices, field_op_types, type_id) *before* assign_variables
    // by doing a lightweight pre-scan of the FB declarations.
    for fb_decl in fb_decls {
        ...
        ctx.user_fb_types.insert(
            ...
            UserFbTypeInfo {
                ...
                var_offset: 0, // placeholder, updated after program vars assigned
                field_op_types,
            },
        );
        next_function_id += 1;

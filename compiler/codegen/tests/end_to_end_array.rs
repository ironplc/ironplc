...
// sum is var index 1 (arr=0, sum=1, i=2). 10+20+30+40+50 = 150.
e2e_i32!(
...
// arr[1] = first of `3(10)` = 10; arr[4] = first of `3(20)` = 20.
e2e_i32!(
...
// result var is at index 0 because function-local `stack` array doesn't
// live in the program's VAR layout.
e2e_i32!(
...
// Multi-scan: x (var 1) remains 99 across two scans once arr[1] was set.
#[test]
fn end_to_end_when_array_persists_across_scans_then_values_retained() {
    let source = "PROGRAM main VAR arr : ARRAY[1..3] OF INT; x : INT; scan_count : INT; END_VAR scan_count := scan_count + 1; IF scan_count = 1 THEN arr[1] := 99; END_IF; x := arr[1]; END_PROGRAM";

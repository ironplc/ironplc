...
e2e_i32!(
    ...
    "PROGRAM main VAR i : INT; sum : DINT; END_VAR FOR i := 1 TO 100 DO sum := sum + 1; END_FOR; END_PROGRAM",
    &[(1, 100)],
);

e2e_i32!(
    end_to_end_when_for_sint_iterates_then_correct_count,
    "PROGRAM main VAR i : SINT; count : DINT; END_VAR FOR i := 1 TO 10 DO count := count + 1; END_FOR; END_PROGRAM",
    &[(1, 10)],
);

e2e_i32!(
    end_to_end_when_for_uint_iterates_then_correct_count,
    "PROGRAM main VAR i : UINT; count : DINT; END_VAR FOR i := 1 TO 50 DO count := count + 1; END_FOR; END_PROGRAM",
    &[(1, 50)],
);

e2e_i32!(
    end_to_end_when_for_int_negative_step_then_correct_count,
    "PROGRAM main VAR i : INT; count : DINT; END_VAR FOR i := 100 TO 1 BY -1 DO count := count + 1; END_FOR; END_PROGRAM",
    &[(1, 100)],
);

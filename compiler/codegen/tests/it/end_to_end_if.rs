...
e2e_i32!(
    ...
    "PROGRAM main VAR x : DINT; y : DINT; END_VAR x := 5; IF x > 0 THEN y := 1; END_IF; END_PROGRAM",
    &[(0, 5), (1, 1)],
);
...
e2e_i32!(
    ...
    "PROGRAM main VAR x : DINT; y : DINT; END_VAR x := 5; IF x > 0 THEN y := 1; ELSE y := 2; END_IF; END_PROGRAM",
    &[(0, 5), (1, 1)],
);
...
e2e_i32!(
    ...
    "PROGRAM main VAR x : DINT; y : DINT; END_VAR x := 10; IF x > 5 THEN y := 1; ELSIF x > 0 THEN y := 2; ELSE y := 3; END_IF; END_PROGRAM",
    &[(0, 10), (1, 1)],
);

e2e_i32!(
    end_to_end_when_if_elsif_else_second_true_then_executes_second,
    "PROGRAM main VAR x : DINT; y : DINT; END_VAR x := 3; IF x > 5 THEN y := 1; ELSIF x > 0 THEN y := 2; ELSE y := 3; END_IF; END_PROGRAM",
    &[(0, 3), (1, 2)],
);

e2e_i32!(
    end_to_end_when_if_elsif_else_none_true_then_executes_else,
    "PROGRAM main VAR x : DINT; y : DINT; END_VAR x := -5; IF x > 5 THEN y := 1; ELSIF x > 0 THEN y := 2; ELSE y := 3; END_IF; END_PROGRAM",
    &[(0, -5), (1, 3)],
);

...
pub fn insert_keyword_statement_terminators(input: Vec<Token>, _file_id: &FileId) -> Vec<Token> {
    ...
    for tok in input {
        ...
        } else if in_end_statement && tok.token_type != TokenType::Semicolon && tok.token_type != TokenType::Comment && tok.token_type != TokenType::Whitespace {
            // TODO remove the span and line/col
            output.push(Token { token_type: TokenType::Semicolon, span: tok.span.clone(), line: tok.line, col: tok.col, text: "".to_owned() });

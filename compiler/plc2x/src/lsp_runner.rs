...
use ironplc_sources::{parse_source, FileType, Source};
...
impl VmRunner {
    /// Compile from a single source string and create a new runner session.
    pub fn load(
        source: &str,
        cycle_time_us: u64,
        ...
        Self::from_bytes(container_bytes, cycle_time_us)
    }

    /// Compile from project sources and create a new runner session.
    pub fn load_from_sources(
        sources: &mut [&mut Source],
        cycle_time_us: u64,
        options: &CompilerOptions,
    ) -> Result<(Self, RunResult), RunResult> {
        let container_bytes = compile_sources_to_bytes(sources, options)?;
        Self::from_bytes(container_bytes, cycle_time_us)
    }

    /// Create runner from compiled bytecode bytes.
    fn from_bytes(
        container_bytes: Vec<u8>,
        cycle_time_us: u64,
    ) -> Result<(Self, RunResult), RunResult> {
...
/// Compile from project sources (already parsed/loaded by the LSP).
fn compile_sources_to_bytes(
    sources: &mut [&mut Source],
    options: &CompilerOptions,
) -> Result<Vec<u8>, RunResult> {
        ...
        match source.library() {
            ...
            Err(diagnostics) => {
                for d in diagnostics {
                    parse_errors.push(d.description());
                }
            }
        }
    }
    ...
    if !parse_errors.is_empty() {
        return Err(RunResult {
            ...
            variables: vec![],
            total_scans: 0,
            error: Some(parse_errors.join("; ")),
        });
    }

    let (library, context) = analyze(&all_libraries, options).map_err(|diagnostics| RunResult {
        ok: false,
        variables: vec![],
        total_scans: 0,
        error: Some(

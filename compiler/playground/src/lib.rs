...
/// Stable codes for host/embedding-layer errors (the `H####` family).
///
/// Generated by `build.rs` from `resources/problem-codes.csv`, the same source
/// the docs build reads to require a reference page per code. `H1xxx` codes are
/// user-facing (bad input the caller can act on); `H9xxx` codes are internal or
/// frontend↔WASM contract violations that should never reach an end user.
mod host_codes {
    include!(concat!(env!("OUT_DIR"), "/host_codes.rs"));
}
...
struct RunError {
    ...
    message: String,
    /// The error's stable code — a VM trap's v-code (e.g. `"V4001"`) or a
    /// host/embedding-layer code (e.g. `"H1001"`; see [`host_codes`]). Every
    /// host-layer error site populates this, so it is only absent on values
...
pub fn compile(source: &str, dialect: &str, allows: &str) -> String {
    ...
    serde_json::to_string(&result).unwrap_or_else(|e| {
        let code = host_codes::SERIALIZATION_ERROR;
        format!(r#"{{"ok":false,"diagnostics":[{{"code":"{code}","message":"Serialization error: {e}","label":"","start_line":1,"start_column":1,"end_line":1,"end_column":1}}]}}"#)
    })
}
...
fn compile_inner(source: &str, dialect: &str, allows: &str) -> CompileResult {
    ...
    if let Err(e) = container.write_to(&mut buf) {
        return CompileResult {
            ...
            bytecode: None,
            diagnostics: vec![DiagnosticInfo {
                code: host_codes::BYTECODE_SERIALIZE_FAILED.to_string(),
...
pub fn run(bytecode_base64: &str, scans: u32) -> String {
    ...
    serde_json::to_string(&result).unwrap_or_else(|e| {
        let code = host_codes::SERIALIZATION_ERROR;
        format!(r#"{{"ok":false,"variables":[],"scans_completed":0,"error":{{"message":"Serialization error: {e}","code":"{code}"}}}}"#)
    })
}
        ...
        Err(e) => {
            return RunResult {
                ...
                error: Some(RunError {
                    ...
                    code: Some(host_codes::INVALID_BASE64.to_string()),
                }),
            };
        }
    };
        ...
        Err(e) => {
            return RunResult {
                ...
                error: Some(RunError {
                    ...
                    code: Some(host_codes::INVALID_CONTAINER.to_string()),
                }),
            };
        }
    };
...
pub fn run_source(source: &str, scans: u32, dialect: &str, allows: &str) -> String {
    ...
    serde_json::to_string(&result).unwrap_or_else(|e| {
        let code = host_codes::SERIALIZATION_ERROR;
        format!(r#"{{"ok":false,"diagnostics":[],"variables":[],"scans_completed":0,"error":{{"message":"Serialization error: {e}","code":"{code}"}}}}"#)
    })
}
...
pub fn load_program(source: &str, cycle_time_us: u32, dialect: &str, allows: &str) -> String {
    ...
    serde_json::to_string(&result).unwrap_or_else(|e| {
        let code = host_codes::SERIALIZATION_ERROR;
        format!(r#"{{"ok":false,"diagnostics":[],"variables":[],"total_scans":0,"error":{{"message":"Serialization error: {e}","code":"{code}"}}}}"#)
    })
}
        ...
        Err(e) => {
            return StepResult {
                ...
                error: Some(RunError {
                    ...
                    code: Some(host_codes::BYTECODE_LOAD_FAILED.to_string()),
                }),
            };
        }
    };
            ...
            None => {
                return StepResult {
                    ...
                    error: Some(RunError {
                        ...
                        code: Some(host_codes::NO_PROGRAM_LOADED.to_string()),
                    }),
                };
            }
        };
        ...
        if session.faulted {
            return StepResult {
                ...
                error: Some(RunError {
                    ...
                    code: Some(host_codes::SESSION_FAULTED.to_string()),
                }),
            };
        }
...
";
    ...
    #[test]
    fn run_when_invalid_base64_then_error_has_host_code() {
        let json = run("not-valid-base64!!!", 1);
        let result: RunResult = serde_json::from_str(&json).unwrap();
        assert!(!result.ok);
        let error = result.error.expect("expected a host error");
        assert_eq!(error.code.as_deref(), Some("H1001"));
        assert!(json.contains("\"code\":\"H1001\""));
    }

    #[test]
    fn run_when_invalid_container_then_error_has_host_code() {
        let bytes = BASE64.encode(b"not a container");
        let result: RunResult = serde_json::from_str(&run(&bytes, 1)).unwrap();
        assert!(!result.ok);
        let error = result.error.expect("expected a host error");
        assert_eq!(error.code.as_deref(), Some("H1002"));
    }
...
";
    ...
    fn step_when_no_session_then_returns_error() {
        ...
        assert_eq!(error.code.as_deref(), Some("H9002"));
    }
...
";
        ...
        assert_eq!(error.code.as_deref(), Some("H9003"));
    }

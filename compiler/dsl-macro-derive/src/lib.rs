...
pub fn recurse_macro_derive(input: TokenStream) -> TokenStream {
    ...
    let res: Result<TokenStream> = match &ast.data {
        syn::Data::Struct(data_struct) => match &data_struct.fields {
            syn::Fields::Named(named_fields) => expand_struct(name, named_fields),
            _ => {
                unimplemented!("#[derive(Recurse)] is only supported for structs with named types")
            }
        },
        syn::Data::Enum(data_enum) => expand_enum(name, data_enum),
        syn::Data::Union(_) => {

...
fn execute_when_jmp_if_not_true_then_no_jump() {
    ...
    assert_eq!(crate::common::run_and_read_i32(&bytecode, 1, &[1, 42]), 42);
}
...
fn execute_when_jmp_if_not_false_then_jumps() {
    ...
    assert_eq!(crate::common::run_and_read_i32(&bytecode, 1, &[0, 42]), 0);
}
...
fn execute_when_if_else_true_then_takes_then_branch() {
    ...
    assert_eq!(
        crate::common::run_and_read_i32(&bytecode, 1, &[1, 10, 20]),
        10
    );
}
...
fn execute_when_if_else_false_then_takes_else_branch() {
    ...
    assert_eq!(
        crate::common::run_and_read_i32(&bytecode, 1, &[0, 10, 20]),
        20
    );
}

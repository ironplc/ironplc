...
impl ExprTypeResolver<'_> {
    ...
    fn insert(&mut self, node: &VarDecl) {
        // Extract array element type when applicable, before the main match
        // may early-return for inline specs.
    ...
    fn element_type_from_named_array(&self, type_name: &TypeName) -> Option<TypeName> {
        ...
        match &attrs.representation {
            IntermediateType::Array { element_type, .. } => self
                .type_environment
                .elementary_type_name_from_repr(element_type),
            _ => None,
        }
    }
        ...
        match var {
            ...
            Variable::Symbolic(SymbolicVariableKind::Deref(deref_var)) => {
                ...
                if let Some(target) = attrs.representation.referenced_type() {
                    self.type_environment.elementary_type_name_from_repr(target)
                } else {
                    None
                }
            }

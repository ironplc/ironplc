...
use ironplc_dsl::{common::{Library, VariableType}, core::{Id, Located}, diagnostic::Diagnostic, visitor::Visitor};
use log::debug;
...
impl<'a> Visitor<Diagnostic> for SymbolEnvironmentResolver<'a> {
    ...
    fn visit_edge_var_decl(
        ...
        self.env.insert(&node.identifier, SymbolKind::EdgeVariable, &self.current_scope())?;
        node.recurse_visit(self)
    }
    ...
    fn visit_data_type_declaration_kind(&mut self, node: &ironplc_dsl::common::DataTypeDeclarationKind) -> Result<Self::Value, Diagnostic> {
        match node {
            ironplc_dsl::common::DataTypeDeclarationKind::Simple(decl) => {
                self.env.insert(&decl.type_name.name, SymbolKind::Type, &ScopeKind::Global)?;
            },
    ...
    fn visit_structure_element_declaration(&mut self, node: &ironplc_dsl::common::StructureElementDeclaration) -> Result<Self::Value, Diagnostic> {
        self.env.insert(&node.name, SymbolKind::StructureElement, &self.current_scope())?;
        node.recurse_visit(self)
    }

    fn visit_enumeration_declaration(&mut self, node: &ironplc_dsl::common::EnumerationDeclaration) -> Result<Self::Value, Diagnostic> {
        // Add the enumeration type itself
        self.env.insert(&node.type_name.name, SymbolKind::Type, &ScopeKind::Global)?;
        
        // Add each enumeration value
        if let ironplc_dsl::common::EnumeratedSpecificationKind::Values(values) = &node.spec_init.spec {
            for value in &values.values {
                self.env.insert(&value.value, SymbolKind::EnumerationValue, &ScopeKind::Global)?;
            }
        }
...
END_FUNCTION_BLOCK";
        ...
        let reset_symbol = env.get(&Id::from("Reset"), &ScopeKind::Named(Id::from("Counter"))).unwrap();
        assert_eq!(reset_symbol.kind, SymbolKind::Parameter);
        
        let count_symbol = env.get(&Id::from("Count"), &ScopeKind::Named(Id::from("Counter"))).unwrap();
        assert_eq!(count_symbol.kind, SymbolKind::Parameter);
        
        // Check that output parameters are captured
        let out_symbol = env.get(&Id::from("OUT"), &ScopeKind::Named(Id::from("Counter"))).unwrap();
        assert_eq!(out_symbol.kind, SymbolKind::OutputParameter);
        
        // Check that local variables are captured
        let cnt_symbol = env.get(&Id::from("Cnt"), &ScopeKind::Named(Id::from("Counter"))).unwrap();

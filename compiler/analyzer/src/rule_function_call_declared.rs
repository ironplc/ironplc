...
mod tests {
    ...
    rule_ctx_ok!(
        ...
        "FUNCTION_BLOCK CALLER VAR result : REAL; value : INT; END_VAR result := INT_TO_REAL(value); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_user_function_called_then_ok,
        "FUNCTION ADD_INTS : INT VAR_INPUT A : INT; B : INT; END_VAR ADD_INTS := A + B; END_FUNCTION FUNCTION_BLOCK CALLER VAR result : INT; END_VAR result := ADD_INTS(1, 2); END_FUNCTION_BLOCK"
    );

    rule_ctx_err1!(
        apply_when_function_not_declared_then_error,
        "FUNCTION_BLOCK CALLER VAR result : INT; END_VAR result := NONEXISTENT_FUNC(1); END_FUNCTION_BLOCK",
        Problem::FunctionCallUndeclared
    );
    ...
    rule_ctx_err1!(
        ...
        "FUNCTION_BLOCK CALLER VAR result : REAL; value : INT; END_VAR result := INT_TO_REAL(value, value); END_FUNCTION_BLOCK",
        Problem::FunctionCallWrongArgCount
    );

    rule_ctx_ok!(
        apply_when_abs_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; value : INT; END_VAR result := ABS(value); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_sqrt_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : REAL; value : REAL; END_VAR result := SQRT(value); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_min_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; END_VAR result := MIN(a, b); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_max_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; END_VAR result := MAX(a, b); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_limit_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; low : INT; value : INT; high : INT; END_VAR result := LIMIT(low, value, high); END_FUNCTION_BLOCK"
    );

    rule_ctx_err1!(
        apply_when_limit_called_with_wrong_arg_count_then_error,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; END_VAR result := LIMIT(a, b); END_FUNCTION_BLOCK",
        Problem::FunctionCallWrongArgCount
    );

    rule_ctx_ok!(
        apply_when_expt_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; base : INT; exp : INT; END_VAR result := EXPT(base, exp); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_mux_called_with_3_args_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; END_VAR result := MUX(0, a, b); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_mux_called_with_5_args_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; c : INT; d : INT; END_VAR result := MUX(2, a, b, c, d); END_FUNCTION_BLOCK"
    );

    rule_ctx_err1!(
        apply_when_mux_called_with_too_few_args_then_error,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; END_VAR result := MUX(0, a); END_FUNCTION_BLOCK",
        Problem::FunctionCallWrongArgCount
    );

    rule_ctx_ok!(
        apply_when_mux_called_with_17_args_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; c : INT; d : INT; e : INT; f : INT; g : INT; h : INT; i : INT; j : INT; k : INT; l : INT; m : INT; n : INT; o : INT; p : INT; END_VAR result := MUX(0, a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p); END_FUNCTION_BLOCK"
    );

    rule_ctx_err1!(
        apply_when_mux_called_with_18_args_then_error,
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; c : INT; d : INT; e : INT; f : INT; g : INT; h : INT; i : INT; j : INT; k : INT; l : INT; m : INT; n : INT; o : INT; p : INT; q : INT; END_VAR result := MUX(0, a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q); END_FUNCTION_BLOCK",
        Problem::FunctionCallWrongArgCount
    );
    ...
    rule_ctx_ok!(
        ...
        "FUNCTION_BLOCK CALLER VAR result : INT; a : INT; b : INT; END_VAR result := ADD(a, b); result := SUB(a, b); result := MUL(a, b); result := DIV(a, b); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_comparison_function_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR result : BOOL; a : INT; b : INT; END_VAR result := GT(a, b); result := GE(a, b); result := EQ(a, b); result := LE(a, b); result := LT(a, b); result := NE(a, b); END_FUNCTION_BLOCK"
    );

    rule_ctx_ok!(
        apply_when_bit_string_conversion_called_then_ok,
        "FUNCTION_BLOCK CALLER VAR b : BYTE; w : WORD; d : DWORD; l : LWORD; END_VAR w := BYTE_TO_WORD(b); d := BYTE_TO_DWORD(b); l := BYTE_TO_LWORD(b); b := WORD_TO_BYTE(w); d := WORD_TO_DWORD(w); l := WORD_TO_LWORD(w); b := DWORD_TO_BYTE(d); w := DWORD_TO_WORD(d); l := DWORD_TO_LWORD(d); b := LWORD_TO_BYTE(l); w := LWORD_TO_WORD(l); d := LWORD_TO_DWORD(l); END_FUNCTION_BLOCK"
    );

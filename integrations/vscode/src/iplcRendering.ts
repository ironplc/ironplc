...
export interface DisassemblyConstant {
  index: number;
  type: string;
  value: string;
}

export interface DisassemblyFunction {
  id: number;
  maxStackDepth: number;
  numLocals: number;
...
export function getOpcodeClass(opcode: string): string {
...
function wrapInHtmlDocument(body: string, extraStyles?: string): string {
...
export function renderHeader(header: DisassemblyHeader): string {
  if (!header) {
    return '';
  }
...
export function renderConstants(constants: DisassemblyConstant[]): string {
...
export function renderFunctions(functions: DisassemblyFunction[]): string {
...
export function getDisassemblyHtml(data: DisassemblyResult): string {
  if (data.error) {
    return getErrorHtml(data.error);
  }

  const headerHtml = renderHeader(data.header);
  const constantsHtml = renderConstants(data.constants);
  const functionsHtml = renderFunctions(data.functions);

  const body = `<h1>IPLC Bytecode Viewer</h1>
  ${headerHtml}
  ${constantsHtml}
  ${functionsHtml}`;

  return wrapInHtmlDocument(body, DISASSEMBLY_STYLES);
}

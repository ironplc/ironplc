...
2. **Parse the `.sln`.** Line-oriented, not XML. Extract every
   `Project("{...}") = "Name", "RelativePath", "{...}"` line via a
   simple line-anchored parse (no need for a general VS solution-file
   parser); keep only entries whose `RelativePath` ends in `.tsproj`
   (case-insensitive), discarding other VS project types
   (`.tcdmproj`, `.tcmproj`, etc.) outright.
...
- `compiler/sources/src/discovery/mod.rs`:
  ...
  - New `parse_sln(sln_path) -> Vec<PathBuf>` -- returns resolved
    `.tsproj` paths.
...
case, not copying its actual proprietary content):

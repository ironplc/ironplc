...
New problem code `P4037` (`AbstractFunctionBlockInstantiated`).
...
| File | Change |
|------|--------|
| `compiler/problems/resources/problem-codes.csv` | New `P4037` |
| `docs/reference/compiler/problems/P4037.rst` | New problem doc |
...
- Semantic tests: declaring a variable of an `ABSTRACT` FB type produces
  `P4037`; declaring a variable of a non-abstract FB type is unaffected;
...
- Verified end-to-end via the CLI: the exact TcXaeShell repro now
  produces `P4037` under `--dialect=codesys` (alongside the pre-existing

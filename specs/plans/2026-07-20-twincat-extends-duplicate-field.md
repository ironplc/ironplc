...
New problem code `P4036` (`ExtendsFieldNameDuplicated`), following the
...
| File | Change |
|------|--------|
| `compiler/problems/resources/problem-codes.csv` | New `P4036` |
| `docs/reference/compiler/problems/P4036.rst` | New problem doc |
...
- Semantic tests: a derived FB redeclaring a base field (same type, and
  a different type) both produce `P4036`; a derived FB with no field
...
- Verified end-to-end via the CLI: the exact TcXaeShell repro now
  produces `P4036` under `--dialect=codesys`, with a message identifying

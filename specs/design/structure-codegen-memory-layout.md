...
7. ~~**Structure type descriptors in the container** — for debugger/verifier support~~ — resolved for the debugger by the COMPOSITE_TYPE debug sub-table ([Variable Inspection Model](variable-inspection-model.md)); the verifier continues to see a structure as a flat SLOT array

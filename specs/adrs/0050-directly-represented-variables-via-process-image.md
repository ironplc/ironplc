# ADR-0050: Directly-Represented Variables via the Process Image
...
records, under *Decisions (settled in review)*: *"Build the process
...
* **Correct IEC I/O semantics** — inputs frozen for the whole scan, outputs
  ...
  express this.
* **Enable the committed roadmap** — FMI co-simulation requires the process
  image; a real I/O driver has nothing to bind to without it.
...
The detailed compiler-side design — address resolution, region layout and
sizing, opcode design, codegen, and the open design questions — lives in
[Directly-Represented Variables Code Generation](../design/directly-represented-variables-codegen.md).
The runtime-side realization (regions, scan-phase wiring, staging/snapshot
semantics) is specified in [Runtime Execution Model](../design/runtime-execution-model.md).
This ADR records only the decision and its rationale.

### Summary of the approach

- Each `AddressAssignment` resolves at compile time to a `(region, byte_offset,
  access_width)` triple — region from the location prefix, width from the size
  prefix, offset from the address vector via the runtime-model formulas.
- Located reads/writes compile to the located-access opcodes
  (`LOAD_INPUT`/`STORE_OUTPUT`/`LOAD_MEMORY`/`STORE_MEMORY`, encoded per
  [ADR-0033](0033-opcode-encoding-by-class-and-type.md)), not the scalar-slot
  opcodes. Named and anonymous forms share one resolver, so the L867 TODO is
  removed as a side effect.
- Semantics follow the runtime model: `%I` is a frozen snapshot, `%Q` is staged
  and flushed atomically (skipped on fault for last-known-good), `%M` is direct
  read-write.
- Located offsets are bounds-checked against region sizes by the bytecode
  verifier ([ADR-0006](0006-bytecode-verification-requirement.md)), analogous to
  [ADR-0023](0023-array-bounds-safety.md).

### The I/O driver stays out of scope

This decision implements the process image and the located-access instruction
...
of scope. The first real driver is the FMI shim; a bare CLI run leaves inputs
zero-filled and outputs observable only via the diagnostic interface.
...
- [Directly-Represented Variables Code Generation](../design/directly-represented-variables-codegen.md)
  — the detailed compiler-side design this decision authorizes.
...
- [FMI Co-Simulation Support](../design/fmi-co-simulation-support.md) — the
  committed consumer of the process image; this work is its phase 2.
